...
sub connect_db {
    #debug(Dumper("dbi:Pg:dbname=".setting('dbname').";host=".setting('dbhost').";port=".setting('dbport').";" . setting('dbuser'). "   ". setting('dbpasswd')));
    my $dbh = DBI->connect("dbi:Pg:dbname=".setting('dbname').";host=".setting('dbhost').";port=".setting('dbport').";", setting('dbuser'), setting('dbpasswd'));
    $dbh->{AutoCommit} = 1;
    $dbh->{RaiseError} = 1;
...
sub get_balance {
    ...
    my $dbh = connect_db();
    my $sql = 'SELECT amount, bxucoin_hash FROM balance where account_id = ?';
...
#    $sql = 'SELECT count(bxucoin_id) FROM bxucoin where account_id = ? and status = ?';
#    $sth = $dbh->prepare($sql);
#    $rv = $sth->execute($account_id, 'valid');
    $sql = 'SELECT amount, bxucoin_hash FROM balance where account_id = ?';
    $sth = $dbh->prepare($sql);
    $rv = $sth->execute($account_id);
    @result = $sth->fetchrow_array;
    my $total = $result[0];

    my $div = int($amount / $RATE);
    my $rem = int($amount % $RATE);
    my $deduce;
    if ($rem == 0) {
        $deduce = $div;
    } else {
...
############ New TMP
    eval {
        ...
        $sql = 'UPDATE balance SET amount = amount - ?, bxucoin_hash = ?, last_update = CURRENT_TIMESTAMP where account_id = ?';
        $sth = $dbh->prepare($sql);
        ...
        $sql = 'INSERT INTO bxucoin_hash_map (bxucoin_hash_id, amount, account_id) VALUES (?, ?, ?)';
        $sth = $dbh->prepare($sql);
...
#    eval {
#
#        $sql = 'SELECT bxucoin_id FROM bxucoin WHERE account_id = ? AND status = ? ORDER BY last_update ASC LIMIT ?';
#        $sth = $dbh->prepare($sql);
#        $rv = $sth->execute($from_id, 'valid', $amount);
#
#        my $sql2 = 'UPDATE bxucoin SET account_id = ? , last_update = CURRENT_TIMESTAMP where bxucoin_id = ?';
#
#
#        while (my @result = $sth->fetchrow_array) {
#            my $bxucoinid = $result[0];
#            my $sth2 = $dbh->prepare($sql2);
#            my $rv2 = $sth2->execute($to_id, $bxucoinid);
#            push @bxucoin_list, $bxucoinid;
#        }
#    }; 
#    if ( $@ ) {
#        $out->{'error'} = "transaction error during processing";
...
#            my $bxucoinid = $result[0];
#            my $sth2 = $dbh->prepare($sql2);
#            my $rv2 = $sth2->execute('88888888-8888-8888-8888-888888888888', $bxucoinid);
#            push @decuce_list, $bxucoinid;
#        }
#    };
#    if ( $@ ) {
#        $out->{'error'} = "transaction error during processing";
...
#    }
# 
#    $reference->{'deduce_bxucoin'} = \@decuce_list;

    eval {
        $sql = 'INSERT INTO transaction (transaction_id, from_id, to_id, bxucoin_data, reference, status, amount, deduce) values (?, ?, ?, ?, ?, ?, ?, ?)';
        $sth = $dbh->prepare($sql);
    ...
    #update_bxucoin_hash($from_id);
    #update_bxucoin_hash($to_id);
...
sub register {
    ...
    eval {
        ...
        $sql = 'INSERT INTO balance (account_id, bxucoin_hash) VALUES (?,?)';
        $sth = $dbh->prepare($sql);
...
#sub update_bxucoin_hash {
#    my ($uid) = @_;
#    my $dbh = connect_db();
#
#    my $sql = 'SELECT bxucoin_id FROM bxucoin WHERE account_id = ? AND status = ?';
#    my $sth = $dbh->prepare($sql);
...
#    }
#
#    my $sha = sha512_hex(@bxucoin_list);
#
#    eval {
#        $sql = 'UPDATE bxucoin_hash SET bxucoin_hash = ?, last_update = CURRENT_TIMESTAMP WHERE uid = ?';    
#        $sth = $dbh->prepare($sql);
#        $rv = $sth->execute($sha, $uid);
#    };
#    if ( $@ ) {
#        $sql = 'INSERT INTO bxucoin_hash (uid, bxucoin_hash) values (?, ?)';
#        $sth = $dbh->prepare($sql);
...
sub get_total_bxucoin {
    #my $memcached = connect_mem();
    my $answer;# = $memcached->get('total-bxucoin');
    ...
    my $count = $sth->fetchrow_array;
    #$memcached->set('total-bxucoin', $count, 3600);
    return $count;
}
